...
def Simulation_Function_germany_Total(dataDF,price_flag,tdp_flag):
    
    DATAF = dataDF.copy()
    DATAF.index = range(len(DATAF))
    SUBCAT = DATAF.SUBCAT_CD.iloc[0]
    price_cfg= pd.read_excel("./gr_models/Germany_Model_Variables.xlsx",sheet_name='PRICE')
    tdp_cfg= pd.read_excel("./gr_models/Germany_Model_Variables.xlsx",sheet_name='TDP')
    sales_cfg= pd.read_excel("./gr_models/Germany_Model_Variables.xlsx",sheet_name='SALES')
    model_cfg= pd.read_excel("./gr_models/Germany_Model_Variables.xlsx",sheet_name='MODEL_NAME')
...
#    DATAF.rename(columns = {'AVG_TEMP_CELSIUS':'AVG(AVG_TEMP_CELSIUS)','HUMID_PCT':'AVG(HUMID_PCT)'},inplace =True)
    ...
    if (price_flag==0):
        ...
        filename = './gr_models/Price_Total_Prediction_'+str(SUBCAT)+"_"+str(CHNL)+"_"+str(FMT)+"_"+str(REG)
    ...
    if tdp_flag==0:    
        ...
        filename = './gr_models/TDP_Total_Prediction_'+str(SUBCAT)+"_"+str(CHNL)+"_"+str(FMT)+"_"+str(REG)
