...
def Simulation_Function_FR_Total(dataDF,price_flag,tdp_flag):
    
    DATAF = dataDF.copy()
    DATAF.index = range(len(DATAF))
    SUBCAT = DATAF.SUBCAT_CD.iloc[0]
    price_cfg= pd.read_excel("./fr_models/France_Model_Variables.xlsx",sheet_name='PRICE')
    tdp_cfg= pd.read_excel("./fr_models/France_Model_Variables.xlsx",sheet_name='TDP')
    sales_cfg= pd.read_excel("./fr_models/France_Model_Variables.xlsx",sheet_name='SALES')
    model_cfg= pd.read_excel("./fr_models/France_Model_Variables.xlsx",sheet_name='MODEL_NAME')
...
#    DATAF.rename(columns = {'AVG_TEMP_CELSIUS':'AVG(AVG_TEMP_CELSIUS)','HUMID_PCT':'AVG(HUMID_PCT)'},inplace =True)
    ...
    if (price_flag==0):
        ...
        DATAF1.columns
        filename = './fr_models/Price_Total_Prediction_'+str(SUBCAT)+"_"+str(CHNL)+"_"+str(FMT)+"_"+str(REG)
    ...
    if tdp_flag==0:    
        ...
        filename = './fr_models/TDP_Total_Prediction_'+str(SUBCAT)+"_"+str(CHNL)+"_"+str(FMT)+"_"+str(REG)
    ...
    if SUBCAT == 1:
        ...
        DATAF.PRICE_PER_VOL = DATAF.PRICE_PER_VOL.shift(0)
        DATAF.fillna(DATAF.PRICE_PER_VOL.mean(),inplace = True)
    elif SUBCAT == 3:
        DATAF.PRICE_PER_VOL = DATAF.PRICE_PER_VOL.shift(-4)
        DATAF.fillna(DATAF.PRICE_PER_VOL.mean(),inplace = True)
    elif SUBCAT == 4:
        DATAF.RESIDENTIAL_PCT_CHANGE = DATAF.RESIDENTIAL_PCT_CHANGE.shift(-1)
        DATAF.fillna(DATAF.PRICE_PER_VOL.mean(),inplace = True)
    elif SUBCAT == 5:
        DATAF.PRICE_PER_VOL = DATAF.PRICE_PER_VOL.shift(-1)
        DATAF.fillna(DATAF.PRICE_PER_VOL.mean(),inplace = True)
    elif SUBCAT == 6:
        DATAF.PRICE_PER_VOL = DATAF.PRICE_PER_VOL.shift(-2)
        DATAF.fillna(DATAF.PRICE_PER_VOL.mean(),inplace = True)
    elif SUBCAT == 7:
        DATAF.PRICE_PER_VOL = DATAF.PRICE_PER_VOL.shift(-3)
        DATAF.fillna(DATAF.PRICE_PER_VOL.mean(),inplace = True)
...
if SUBCAT in [1]:
    ...
    testing_data = pd.read_csv('FRANCE_toothpaste_TOTAL_LEVEL_BASELINE_SALES_SCENARIO_1.csv')
elif SUBCAT in [3]: 
...
testing_data=testing_data[testing_data['SALES_VOLUME']==0]
...
output = Simulation_Function_FR_Total(testing_data,0,0)
