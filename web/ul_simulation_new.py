...
def get_sales_data(selectq1,selectq2,selectq3,selectq4,input_values):
    ...
    select_query ="""select MONTH_YEAR, """ +selectq1+""" ,RECORD_TYPE
            ...
            join """+hana_db+""".subcat_type b on a.subcat_cd = b.subcat_cd and b.geo_id = ?
...
def ul_db_simulation_hist_pred_data(parameters):
    """
    ...
    if forecast_type == 'total':    
        ...
        for j in range(0, input_var_len):
            if j == input_var_len -1:
                ...
                input_selectQ1 = input_selectQ1 + calc_metric[i]+"(IFNULL(cast(JSON_VALUE(a.RECORDSET_CONTENTS_JSON, '$."+input_var_ui[j]+"') as decimal),0)) as "+input_var_ui[j]
            else:
                input_selectQ = input_selectQ + input_var_ui[j]+","
                input_selectQ1 = input_selectQ1 + calc_metric[i]+"(IFNULL(cast(JSON_VALUE(a.RECORDSET_CONTENTS_JSON, '$."+input_var_ui[j]+"') as decimal),0)) as "+input_var_ui[j]+","
...
#             resultDF1["VOLUME"] = resultDF1["VOLUME"]*exch_rate
        ...
        # to check output
        resultDF2.to_csv("input_table_ROI.csv")
        #resultDF1.to_csv("Sales_volume.csv")
...
#        sys.exit()
    ...
    updated_df_col_list = list(updated_df.columns)
...
def dummy_gen_sample_data():
    ...
    #cat_content = {"COUNTRY_NAME":"142","DIVISION":"1","CATAGORY_NAME":"1","SUB_CATEGORY_NAME":"10","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1","CHANNEL":"0","REGION":"0","FORMAT":"0"}
    ...
    df1= pd.read_excel("simulation_demo_uk.xlsx",sheet_name="INPUT_VALUES")
    sales_vol_dict = {"INPUT_VALUES":(json.loads(df1.to_json(orient='records')))}
    my_dict2.update(sales_vol_dict)
    
    df2= pd.read_excel("simulation_demo_uk.xlsx",sheet_name="TABLE_VALUES")
...
#print(ret2)
#output1.to_csv("model2.csv")
#finalDF.to_csv("input2.csv")
