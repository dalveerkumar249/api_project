...
def Simulation_Function_PH_Total(dataDF,price_flag,tdp_flag):
    ...
    price_cfg= pd.read_excel("./ph_models/PH_Model_Variables.xlsx",sheet_name='PRICE')
    tdp_cfg= pd.read_excel("./ph_models/PH_Model_Variables.xlsx",sheet_name='TDP')
    sales_cfg= pd.read_excel("./ph_models/PH_Model_Variables.xlsx",sheet_name='SALES')
    model_cfg= pd.read_excel("./ph_models/PH_Model_Variables.xlsx",sheet_name='MODEL_NAME')
...
#    DATAF.rename(columns = {'AVG_TEMP_CELSIUS':'AVG(AVG_TEMP_CELSIUS)','HUMID_PCT':'AVG(HUMID_PCT)'},inplace =True)
    ...
    if (price_flag==0):
        ...
        filename = './ph_models/Price_Total_Prediction_'+str(SUBCAT)+"_"+str(CHNL)+"_"+str(FMT)+"_"+str(REG)
    ...
    if tdp_flag==0:    
        ...
        filename = './ph_models/TDP_Total_Prediction_'+str(SUBCAT)+"_"+str(CHNL)+"_"+str(FMT)+"_"+str(REG)
...
testing_data = pd.read_csv('DATAF_ALL_HIST_PROJ_SALES_SCENARIO_ALL_SUBCAT_1_2.csv')

testing_data = pd.read_csv('DATAF_ALL_HIST_PROJ_SALES_SCENARIO_ALL_SUBCAT_3.csv')

#testing_data = pd.read_csv('TotalSalesALLScenarious.csv')
...
output = Simulation_Function_PH_Total(testing_data,0,0)
