...
def get_global_sales_data(input_values):
    ...
    # select_query = """select ctry_name, div_desc, catg_desc, subcat_desc, chnl_desc,global_chnl_name,rollup_chnl, MONTH_YEAR, sum(SALES_VALUE) from 
    #                 (select f.ctry_name,c.div_desc,e.catg_desc,b.subcat_desc,g.chnl_desc,g.global_chnl_name, g.rollup_chnl,
    #                 quarter(a.period_ending_date) as MONTH_YEAR,
    ...
    #                 and year(a.period_ending_date) in (?, ?, ?, ?) 
    #                 group by b.subcat_desc, e.catg_desc, f.ctry_name, c.div_desc, g.chnl_desc, g.global_chnl_name, g.rollup_chnl, quarter(a.period_ending_date) 
    #                 union 
    #                 select f.ctry_name, c.div_desc, e.catg_desc, b.subcat_desc, g.chnl_desc, g.global_chnl_name, g.rollup_chnl,
    #                 quarter(a.period_ending_date) as MONTH_YEAR,
    ...
    #                 join """+hana_db+""".sector_scenario_type d on a.SECTOR_SCENARIO_CD = d.SECTOR_SCENARIO_CD 
    #                 join """+hana_db+""".channel_type g on a.chnl_cd = g.chnl_cd and g.geo_id = z.ctry_geo_id
    #                 --join """+hana_db+""".res_recordset_save aa on a.ul_geo_id = aa.ul_geo_id and a.catg_cd = aa.catg_cd
    #                 --and aa.chnl_cd = 0 and aa.fmt_cd = 0 and aa.subcat_cd = a.subcat_cd
    #                 --and aa.SECTOR_SCENARIO_CD = a.SECTOR_SCENARIO_CD and aa.period_ending_date = a.period_ending_date
    #                 where JSON_VALUE(a.RECORDSET_CONTENTS_JSON, '$.RECORD_TYPE') = 'FORECAST' 
    #                 and a.fmt_cd =0 
    #                 and a.SECTOR_SCENARIO_CD = ?
    #                 and year(a.period_ending_date) in (?, ?, ?, ?)  
    #                 group by b.subcat_desc, e.catg_desc, f.ctry_name, c.div_desc, chnl_desc, g.global_chnl_name,g.rollup_chnl,quarter(a.period_ending_date) 
    #                 ) 
    #                 group by MONTH_YEAR, ctry_name, div_desc, catg_desc, subcat_desc, chnl_desc,global_chnl_name,rollup_chnl
    ...
    select_query = """select ctry_name, div_desc, catg_desc, subcat_desc, chnl_desc,global_chnl_name,rollup_chnl, MONTH_YEAR, sum(SALES_VALUE) from 
                    ...
                    group by b.subcat_desc, e.catg_desc, f.ctry_name, c.div_desc, g.chnl_desc, g.global_chnl_name, g.rollup_chnl, CONCAT(year(a.PERIOD_ENDING_DATE), '-Q', quarter(a.PERIOD_ENDING_DATE)) 
                    union 
                    select f.ctry_name, c.div_desc, e.catg_desc, b.subcat_desc, g.chnl_desc, g.global_chnl_name, g.rollup_chnl,
                    CONCAT(year(a.PERIOD_ENDING_DATE), '-Q', quarter(a.PERIOD_ENDING_DATE)) as MONTH_YEAR,
                    SUM(cast(JSON_VALUE(a.RECORDSET_CONTENTS_JSON, '$[*].PREDICTED_VOLUME') as decimal)* cast(JSON_VALUE(a.RECORDSET_CONTENTS_JSON, '$[*].PRICE_PER_VOL') as decimal)) as SALES_VALUE 
                    ...
                    and year(a.period_ending_date) in (%s, %s, %s, %s)  
                    group by b.subcat_desc, e.catg_desc, f.ctry_name, c.div_desc, chnl_desc, g.global_chnl_name,g.rollup_chnl,CONCAT(year(a.PERIOD_ENDING_DATE), '-Q', quarter(a.PERIOD_ENDING_DATE)) 
                    ) sub
                    group by MONTH_YEAR, ctry_name, div_desc, catg_desc, subcat_desc, chnl_desc,global_chnl_name,rollup_chnl
                    order by ctry_name, div_desc, subcat_desc, chnl_desc, MONTH_YEAR
                    """
...
def currenty_conversion(tempDF,country):
    ...
    for i in range (6,len(col_names)):
        tempDF[col_names[i]] = round((tempDF[col_names[i]]/1000)*exch_rate,2)
...
def ul_db_excel_summary_report(parameters):
    ...
    column_names = ["COUNTRY","DIVISION","CATEGORY","SUB_CATEGORY","CHANNEL","GLOBAL_CHANNEL","ROLLUP_CHNL","MONTH_YEAR","VALUE"]
    ...
    #baseline_DF["VALUE"] = baseline_DF["VALUE"]/1000
    
    longcovid_DF_new["VALUE"] = longcovid_DF_new["VALUE"].astype(float)
    #longcovid_DF_new["VALUE"] = longcovid_DF_new["VALUE"]/1000
    
    roi_DF_new["VALUE"] = roi_DF_new["VALUE"].astype(float)
    #roi_DF_new["VALUE"] = roi_DF_new["VALUE"]/1000
    
    
    #print("\nPivoting...")
    baselinep_DF = pd.pivot_table(baseline_DF,index=["COUNTRY","DIVISION","CATEGORY","SUB_CATEGORY","CHANNEL","GLOBAL_CHANNEL","ROLLUP_CHNL"],columns='MONTH_YEAR',values='VALUE')
    longcovidp_DF = pd.pivot_table(longcovid_DF_new,index=["COUNTRY","DIVISION","CATEGORY","SUB_CATEGORY","CHANNEL","GLOBAL_CHANNEL","ROLLUP_CHNL"],columns='MONTH_YEAR',values='VALUE')
    roip_DF = pd.pivot_table(roi_DF_new,index=["COUNTRY","DIVISION","CATEGORY","SUB_CATEGORY","CHANNEL","GLOBAL_CHANNEL","ROLLUP_CHNL"],columns='MONTH_YEAR',values='VALUE')
    ...
    for country in country_list:
        ...
        if tempDF.shape[0] > 1:
                ...
                if tempDF2. shape[0] >1:
                    ...
                    for catg_name in category_list:
                        ...
                        if tempDF3.shape[0] >1:
                            ...
                            for sub_catg_name in subcat_list:
                                tempDF3a = tempDF3[tempDF3['SUB_CATEGORY']==sub_catg_name].copy()
                                if tempDF3a.shape[0] > 0:
                                    df_len = max(tempDF3a.index)+1
                                    if country == 'Netherlands' and sub_catg_name in['MEAL SOLUTIONS','MEAT','MEAT SUBSTITUTES','SOUP']:
                                        tempDF3a.loc[df_len] = tempDF3a.sum(numeric_only=True, axis=0) 
                                    else:    
                                        tempDF3a.loc[df_len] = tempDF3a[tempDF3a["ROLLUP_CHNL"]==0].sum(numeric_only=True, axis=0) 
                                     
                                    tempDF3a.at[df_len, 'COUNTRY'] = country
                                    tempDF3a.at[df_len, 'DIVISION'] = div_name
                                    tempDF3a.at[df_len, 'CATEGORY'] = catg_name
                                    tempDF3a.at[df_len, 'SUB_CATEGORY'] = sub_catg_name
                                    tempDF3a.at[df_len, 'CHANNEL'] = "Total"
                                    tempDF3a.at[df_len, 'GLOBAL_CHANNEL'] = "Total"
                                    
                                    if final_DF.shape[0] < 1:
                                        final_DF = tempDF3a.copy()
                                    else:
                                        final_DF = final_DF.append(tempDF3a)
                            ...
                            if final_DF.shape[0] < 1:
                                ...
                                if country == 'Netherlands' and sub_catg_name in['MEAL SOLUTIONS','MEAT','MEAT SUBSTITUTES','SOUP']:
                                    ...
                                    tempDF3.loc[df_len] = tempDF3[tempDF3["ROLLUP_CHNL"]==0].sum(numeric_only=True, axis=0) 
                                ...
                                tempDF3.at[df_len, 'CHANNEL'] = "Total"
                                tempDF3.at[df_len, 'GLOBAL_CHANNEL'] = "Total"
                                final_DF = tempDF3.copy()
                            else:
                                df_len = max(final_DF.index)+1
                                if country == 'Netherlands' and sub_catg_name in['MEAL SOLUTIONS','MEAT','MEAT SUBSTITUTES','SOUP']:
                                    final_DF.loc[df_len] = tempDF3.sum(numeric_only=True, axis=0) 
                                else:    
                                    final_DF.loc[df_len] = tempDF3[tempDF3["ROLLUP_CHNL"]==0].sum(numeric_only=True, axis=0) 
                            
                                final_DF.at[df_len, 'COUNTRY'] = country
                                final_DF.at[df_len, 'DIVISION'] = div_name
                                final_DF.at[df_len, 'CATEGORY'] = catg_name
                                final_DF.at[df_len, 'SUB_CATEGORY'] = "Total"
                                final_DF.at[df_len, 'CHANNEL'] = "Total"
                                final_DF.at[df_len, 'GLOBAL_CHANNEL'] = "Total"
            ...
            for div_name in div_list:
                ...
                if tempDF4. shape[0] >1:
                    df_len = max(final_DF.index)+1
                    final_DF.loc[df_len] = tempDF4[tempDF4["SUB_CATEGORY"]=="Total"].sum(numeric_only=True, axis=0) 
                    final_DF.at[df_len, 'COUNTRY'] = country
                    final_DF.at[df_len, 'DIVISION'] = div_name
                    ...
                    tempDF4.at[df_len, 'GLOBAL_CHANNEL'] = "Total"
        ...
        if tempDF.shape[0] >=1:
            tempDF = final_DF[final_DF['COUNTRY']==country].copy()
            df_len = max(final_DF.index)+1
            final_DF.loc[df_len] = tempDF[tempDF["CATEGORY"]=="Total"].sum(numeric_only=True, axis=0) 
            final_DF.at[df_len, 'COUNTRY'] = country
    ...
    final_DF2.at[df_len, 'GLOBAL_CHANNEL'] = "Total"
    ...
    final_DF2["BS_H1_"+ next_year +"_"+curr_year] = (((final_DF2["BS_"+ next_year +"_Q1"] + final_DF2["BS_"+ next_year +"_Q2"])/(final_DF2["BS_"+curr_year +"_Q1"] + final_DF2["BS_"+curr_year +"_Q2"]))-1)*100
    final_DF2["BS_H2_"+ next_year +"_"+curr_year] = (((final_DF2["BS_"+ next_year +"_Q3"] + final_DF2["BS_"+ next_year +"_Q4"])/(final_DF2["BS_"+curr_year +"_Q3"] + final_DF2["BS_"+curr_year +"_Q4"]))-1)*100
    final_DF2["BS_FY_"+curr_year +"_"+pre_year2] = (((final_DF2["BS_"+curr_year +"_Q1"] + final_DF2["BS_"+curr_year +"_Q2"]+final_DF2["BS_"+curr_year +"_Q3"] + final_DF2["BS_"+curr_year +"_Q4"])/(final_DF2["A_"+pre_year2 +"_Q1"] + final_DF2["A_"+pre_year2 +"_Q2"] + final_DF2["A_"+pre_year2 +"_Q3"] + final_DF2["A_"+pre_year2 +"_Q4"]))-1)*100
    final_DF2["BS_FY_"+ next_year +"_"+pre_year1] = (((final_DF2["BS_"+ next_year +"_Q1"] + final_DF2["BS_"+ next_year +"_Q2"]+final_DF2["BS_"+ next_year +"_Q3"] + final_DF2["BS_"+ next_year +"_Q4"])/(final_DF2["A_"+pre_year1+"_Q1"] + final_DF2["A_"+pre_year1+"_Q2"] + final_DF2["A_"+pre_year1+"_Q3"] + final_DF2["A_"+pre_year1+"_Q4"]))-1)*100
    final_DF2["BS_FY_"+ next_year +"_"+curr_year] = (((final_DF2["BS_"+ next_year +"_Q1"] + final_DF2["BS_"+ next_year +"_Q2"]+final_DF2["BS_"+ next_year +"_Q3"] + final_DF2["BS_"+ next_year +"_Q4"])/(final_DF2["BS_"+curr_year +"_Q1"] + final_DF2["BS_"+curr_year +"_Q2"] + final_DF2["BS_"+curr_year +"_Q3"] + final_DF2["BS_"+curr_year +"_Q4"]))-1)*100
    
    
    #print("\nCapturing long covid growth numbers ...")
    # capture long covid growth numbers 
    final_DF2["LC_H1_"+ next_year +"_"+curr_year] = (((final_DF2["LC_"+ next_year +"_Q1"] + final_DF2["LC_"+ next_year +"_Q2"])/(final_DF2["LC_"+curr_year +"_Q1"] + final_DF2["LC_"+curr_year +"_Q2"]))-1)*100
    final_DF2["LC_H2_"+ next_year +"_"+curr_year] = (((final_DF2["LC_"+ next_year +"_Q3"] + final_DF2["LC_"+ next_year +"_Q4"])/(final_DF2["LC_"+curr_year +"_Q3"] + final_DF2["LC_"+curr_year +"_Q4"]))-1)*100
    final_DF2["LC_FY_"+curr_year +"_"+pre_year2] = (((final_DF2["LC_"+curr_year +"_Q1"] + final_DF2["LC_"+curr_year +"_Q2"]+final_DF2["LC_"+curr_year +"_Q3"] + final_DF2["LC_"+curr_year +"_Q4"])/(final_DF2["A_"+pre_year2 +"_Q1"] + final_DF2["A_"+pre_year2 +"_Q2"] + final_DF2["A_"+pre_year2 +"_Q3"] + final_DF2["A_"+pre_year2 +"_Q4"]))-1)*100
    final_DF2["LC_FY_"+ next_year +"_"+pre_year1] = (((final_DF2["LC_"+ next_year +"_Q1"] + final_DF2["LC_"+ next_year +"_Q2"]+final_DF2["LC_"+ next_year +"_Q3"] + final_DF2["LC_"+ next_year +"_Q4"])/(final_DF2["A_"+pre_year1+"_Q1"] + final_DF2["A_"+pre_year1+"_Q2"] + final_DF2["A_"+pre_year1+"_Q3"] + final_DF2["A_"+pre_year1+"_Q4"]))-1)*100
    final_DF2["LC_FY_"+ next_year +"_"+curr_year] = (((final_DF2["LC_"+ next_year +"_Q1"] + final_DF2["LC_"+ next_year +"_Q2"]+final_DF2["LC_"+ next_year +"_Q3"] + final_DF2["LC_"+ next_year +"_Q4"])/(final_DF2["LC_"+curr_year +"_Q1"] + final_DF2["LC_"+curr_year +"_Q2"] + final_DF2["LC_"+curr_year +"_Q3"] + final_DF2["LC_"+curr_year +"_Q4"]))-1)*100
    
    #print("\nCapturing ROI scenario growth numbers ...")
    # capture ROI scenario growth numbers 
    final_DF2["RI_H1_"+ next_year +"_"+curr_year] = (((final_DF2["RI_"+ next_year +"_Q1"] + final_DF2["RI_"+ next_year +"_Q2"])/(final_DF2["RI_"+curr_year +"_Q1"] + final_DF2["RI_"+curr_year +"_Q2"]))-1)*100
    final_DF2["RI_H2_"+ next_year +"_"+curr_year] = (((final_DF2["RI_"+ next_year +"_Q3"] + final_DF2["RI_"+ next_year +"_Q4"])/(final_DF2["RI_"+curr_year +"_Q3"] + final_DF2["RI_"+curr_year +"_Q4"]))-1)*100
    final_DF2["RI_FY_"+curr_year +"_"+pre_year2] = (((final_DF2["RI_"+curr_year +"_Q1"] + final_DF2["RI_"+curr_year +"_Q2"]+final_DF2["RI_"+curr_year +"_Q3"] + final_DF2["RI_"+curr_year +"_Q4"])/(final_DF2["A_"+pre_year2 +"_Q1"] + final_DF2["A_"+pre_year2 +"_Q2"] + final_DF2["A_"+pre_year2 +"_Q3"] + final_DF2["A_"+pre_year2 +"_Q4"]))-1)*100
    final_DF2["RI_FY_"+ next_year +"_"+pre_year1] = (((final_DF2["RI_"+ next_year +"_Q1"] + final_DF2["RI_"+ next_year +"_Q2"]+final_DF2["RI_"+ next_year +"_Q3"] + final_DF2["RI_"+ next_year +"_Q4"])/(final_DF2["A_"+pre_year1+"_Q1"] + final_DF2["A_"+pre_year1+"_Q2"] + final_DF2["A_"+pre_year1+"_Q3"] + final_DF2["A_"+pre_year1+"_Q4"]))-1)*100
    final_DF2["RI_FY_"+ next_year +"_"+curr_year] = (((final_DF2["RI_"+ next_year +"_Q1"] + final_DF2["RI_"+ next_year +"_Q2"]+final_DF2["RI_"+ next_year +"_Q3"] + final_DF2["RI_"+ next_year +"_Q4"])/(final_DF2["RI_"+curr_year +"_Q1"] + final_DF2["RI_"+curr_year +"_Q2"] + final_DF2["RI_"+curr_year +"_Q3"] + final_DF2["RI_"+curr_year +"_Q4"]))-1)*100
    
    final_DF2.replace({-100:0},inplace=True)
    final_DF2.drop(['ROLLUP_CHNL'],inplace=True,axis=1)
    #print("\nfinal_DF2 saved to: summary_channel.csv")
    #final_DF2.to_excel("summary_Report_channel.xlsx")
    
    colnames = final_DF2.columns
    final_DF2[colnames[6:]] = final_DF2[colnames[6:]].round(2)
