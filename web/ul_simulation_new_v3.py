...
import Simulation_function_Indonesia_v1
import Simulation_Function_India_Total
import Simulation_function_UK
import Simulation_function_SA
import simulation_calibration_v1
import Simulation_function_BR
#import Simulation_function_NTH
import Simulation_function_neatherlands
import Simulation_function_MX
import Simulation_function_GERMANY
import Simulation_function_PH
import Simulation_function_FRANCE
import Simulation_function_China
...
def get_granular_sales_data(selectq1,selectq2,joinq1,condition1,grouby1,input_values):
    
    select_query =""" select """+selectq1+""", MONTH_YEAR,SALES_VOLUME,SALES_VALUE,RECORD_TYPE
                    FROM
                    (select """+selectq2+""",
...
def get_input_variables_data_bkup(selectq,selectq1,inp_date,input_values):
    ...
    select_query = """  select MONTH_YEAR ,"""+selectq+"""
                        ...
                        """+selectq1+"""
                        FROM """+hana_db+""".res_recordset_save a
                        JOIN """+hana_db+""".CATEGORY_TYPE c ON c.CATG_CD = a.CATG_CD and DIV_CD = ?
                        join """+hana_db+""".subcat_type b on a.subcat_cd = b.subcat_cd and b.geo_id = ? and b.catg_cd = c.catg_cd
                        join """+hana_db+""".ul_geo_hier z on a.ul_geo_id = z.ul_rgn_geo_id and z.ctry_geo_id = ? and z.ul_region_name = 'Country'
                        join """+hana_db+""".sector_scenario_type d on a.SECTOR_SCENARIO_CD = d.SECTOR_SCENARIO_CD
                        where JSON_VALUE(a.RECORDSET_CONTENTS_JSON, '$.RECORD_TYPE') = 'ACTUAL'
                        AND a.CATG_CD = ?
                        AND a.SUBCAT_CD = ?
...
def ul_db_get_predict_date_new(input_values):
...
#input_values = (2,142,142,9,3)
#predict_date = ul_db_get_predict_date_new(input_values)
...
def ul_get_granular_data(geo_id,division,catg_cd,sub_catg_cd,forecast_scenario):
    
    
    channel_column_names = ["CHANNEL","MONTH_YEAR","VOLUME","VALUE","DATA_TYPE"]
    
    channel_input_values = (division,geo_id,geo_id,geo_id,catg_cd,sub_catg_cd,division,geo_id,geo_id,geo_id,catg_cd,sub_catg_cd,forecast_scenario)
    ...
    channel_df = pd.DataFrame(get_granular_sales_data(selectq1,selectq2,joinq1,condition1,grouby1,channel_input_values),columns=channel_column_names)
    
    if channel_df.shape[0] < 1:
        channel_df = pd.DataFrame()
    else:
        #channel_df["MONTH_YEAR"] = channel_df["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
        channel_df["VOLUME"] = channel_df["VOLUME"].astype(float).round(2)
        channel_df["VALUE"] = channel_df["VALUE"].astype(float).round(2)
        channel_df["DATA_TYPE"] = channel_df["DATA_TYPE"].str.replace('FORECAST','FORECASTED')
        
    region_column_names = ["REGION","MONTH_YEAR","VOLUME","VALUE","DATA_TYPE"]
    region_input_values = (division,geo_id,geo_id,catg_cd,sub_catg_cd,division,geo_id,geo_id,catg_cd,sub_catg_cd,forecast_scenario)
    ...
    results_df1 = pd.DataFrame(get_granular_sales_data(selectq1,selectq2,joinq1,condition1,grouby1,region_input_values),columns=region_column_names)
    ...
    if len(rows) > 0:
        #print("getting overlapping region..")
        ...
        results_df2 = pd.DataFrame(get_granular_sales_data(selectq1,selectq2,joinq1,condition1,grouby1,region_input_values),columns=region_column_names)
        #print(results_df2.shape)
        regions_df = results_df1.append(results_df2,ignore_index=True)
        
    else:
        regions_df = results_df1

    if regions_df.shape[0] < 1:
        regions_df = pd.DataFrame() 
    else:
        #regions_df["MONTH_YEAR"] = regions_df["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
        regions_df["VOLUME"] = regions_df["VOLUME"].astype(float).round(2)
        regions_df["VALUE"] = regions_df["VALUE"].astype(float).round(2)
        regions_df["DATA_TYPE"] = regions_df["DATA_TYPE"].str.replace('FORECAST','FORECASTED')
        
    format_input_values = (division,geo_id,geo_id,geo_id,catg_cd,sub_catg_cd,catg_cd,sub_catg_cd,division,geo_id,geo_id,geo_id,catg_cd,sub_catg_cd,catg_cd,sub_catg_cd,forecast_scenario)
    
    format_column_names = ["FORMAT","MONTH_YEAR","VOLUME","VALUE","DATA_TYPE"]
    ...
    format_df = pd.DataFrame(get_granular_sales_data(selectq1,selectq2,joinq1,condition1,grouby1,format_input_values),columns=format_column_names)

    if format_df.shape[0] < 1:
        format_df = pd.DataFrame() 
    else:
        #format_df["MONTH_YEAR"] = format_df["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
        format_df["VOLUME"] = format_df["VOLUME"].astype(float).round(2)
        format_df["VALUE"] = format_df["VALUE"].astype(float).round(2)
        format_df["DATA_TYPE"] = format_df["DATA_TYPE"].str.replace('FORECAST','FORECASTED')


    return channel_df,format_df,regions_df


##########################################
# unit testing

#chnl_df,fmt_df,regn_df = ul_get_granular_data("142","1","8","1","2")
#chnl_df,fmt_df,regn_df = ul_get_granular_data("73","1","1","6","5")
#chnl_df,fmt_df,regn_df = ul_get_granular_data("181","2","9","2","5")
...
def ul_db_simulation_hist_pred_data(parameters,output=0):
    ...
    if (str(geo_id)=='245') and (output ==1):
        input_var_ui = input_var_ui + ['PRICE_PER_VOL_online','PRICE_PER_VOL_offline']
        calc_metric = calc_metric + ["avg","avg"]
        #print(input_var_ui)
    ...
    if pred_date ==0:     
        ...
        input_var_ui = None
        input_var_ui_pref = None
        calc_metric = None
        calc_metric_pref = None
    ...
    if forecast_type == 'total':    
        ...
        #to get input variables for forecast period
        resultDF2 = pd.DataFrame(get_input_variables_data(input_selectQ,input_selectQ1,pred_date,input_values3),columns=column_names2)
        
        #to get input variables for quarterly growth calculation 
        resultDF3 = pd.DataFrame(get_input_variables_data(input_selectQ,input_selectQ1,comp_date,input_values3),columns=column_names2)
        #print(comp_date)
        #print(resultDF3)
...
#             resultDF1["VOLUME"] = resultDF1["VOLUME"]*exch_rate
        ...
        if str(geo_id) == '245' and str(sub_catg_cd) in['3','4']:
            resultDF2.drop(["TDP"],axis =1,inplace=True)
            resultDF3.drop(["TDP"],axis =1,inplace=True)
        ...
        # to get granular level data for this sub cat
        
        channel_DF,format_DF, region_DF = ul_get_granular_data(geo_id,division,catg_cd,sub_catg_cd,forecast_scenario)
        
        if channel_DF.shape[0] >1:
            channel_DF["MONTH_YEAR"] = channel_DF["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
            channel_DF["VALUE"] = channel_DF["VALUE"]*exch_rate
            channel_DF["VALUE"] = channel_DF["VALUE"].astype(float).round(2)
        
        if format_DF.shape[0] >1:
            format_DF["MONTH_YEAR"] = format_DF["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
            format_DF["VALUE"] = format_DF["VALUE"]*exch_rate
            format_DF["VALUE"] = format_DF["VALUE"].astype(float).round(2)
        
        if region_DF.shape[0] >1:
            region_DF["MONTH_YEAR"] = region_DF["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
            region_DF["VALUE"] = region_DF["VALUE"]*exch_rate
            region_DF["VALUE"] = region_DF["VALUE"].astype(float).round(2)
        ...
        channelDict4 = channel_DF.to_dict(orient='records')
        formatDict5 = format_DF.to_dict(orient='records')
        regionDict6 = region_DF.to_dict(orient='records')
        
        
        responseData ={"request_header":parameters,
                       "SALES_VOLUME": responseDict1,
                       "CHANNEL":channelDict4,
                       "FORMAT":formatDict5,
                       "REGION":regionDict6,
...
#input_vaiables= {"COUNTRY_NAME":"142","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"1","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"73","DIVISION":"1","CATAGORY_NAME":"1","SUB_CATEGORY_NAME":"6","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"181","DIVISION":"2","CATAGORY_NAME":"4","SUB_CATEGORY_NAME":"1","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"179","DIVISION":"2","CATAGORY_NAME":"5","SUB_CATEGORY_NAME":"11","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"170","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"1","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"176","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"5","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"175","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"5","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"172","DIVISION":"3","CATAGORY_NAME":"10","SUB_CATEGORY_NAME":"2","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"177","DIVISION":"1","CATAGORY_NAME":"1","SUB_CATEGORY_NAME":"1","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":"169","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"1","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
#input_vaiables= {"COUNTRY_NAME":245,"DIVISION":1,"CATAGORY_NAME":2,"SUB_CATEGORY_NAME":3,"METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}

#input_vaiables= {"COUNTRY_NAME":"176","DIVISION":"3","CATAGORY_NAME":"15","SUB_CATEGORY_NAME":"4","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
...
#        sys.exit()
    ...
    if str(geo_id) == '245':
        ...
        value4 = input_df['TRAFFIC_WEIGHT'].sum()
        value5 = input_df['TRAFFIC_WEIGHT'].sum()
        input_df['PRICE_PER_VOL_online'] = input_df['PRICE_PER_VOL']
        input_df['PRICE_PER_VOL_offline'] = input_df['PRICE_PER_VOL']
    else:
...
#    # china country 
#    
#    china_input_masterDF = pd.DataFrame()
#    for row in input_df.itertuples(index=False):
#        qtr = getattr(row,'QUARTER')
#        #print(row)
#        if qtr =='Q1':
#            returnDF1 =pd.DataFrame(generate_decompose_dict(Q1,input_df_col_list,row))
#            #print(returnDF1)
#            china_input_masterDF= china_input_masterDF.append(returnDF1,ignore_index=True)
#        if qtr =='Q2':
#            returnDF2 =pd.DataFrame(generate_decompose_dict(Q2,input_df_col_list,row))
#            china_input_masterDF= china_input_masterDF.append(returnDF2,ignore_index=True)
#        if qtr =='Q3':
#            returnDF3 =pd.DataFrame(generate_decompose_dict(Q3,input_df_col_list,row))
#            china_input_masterDF = china_input_masterDF.append(returnDF3,ignore_index=True)
#        if qtr =='Q4':
#            returnDF4 =pd.DataFrame(generate_decompose_dict(Q4,input_df_col_list,row))
#            china_input_masterDF= china_input_masterDF.append(returnDF4,ignore_index=True)
#            
#    china_input_masterDF["PERIOD_ENDING_DATE"] = china_input_masterDF["PERIOD_ENDING_DATE"].astype(str)
#    china_input_masterDF.set_index(['PERIOD_ENDING_DATE'],inplace=True)
    ...
    #Sanjay
    master_flag_df = input_masterDF.copy(deep=True)
    master_flag_df["ROW_SUM"] = master_flag_df.sum(axis=1)
    #print("master_flag_df:")
    #print(master_flag_df.dtypes)
    #master_flag_df.to_csv("san_master_flag_df.csv")
    
    input_values2 = (division,geo_id,geo_id,catg_cd,sub_catg_cd)
    pred_date, comp_date = ul_db_get_predict_date(input_values2)
    
    predict_date = str(ul_db_get_predict_date_new(input_values2))
    ...
    if (str(geo_id)=='245'):
        input_var_ui_rs = input_var_ui_rs + ['PRICE_PER_VOL_online','PRICE_PER_VOL_offline']
    ...
    finalDF = masterDF3[masterDF3['PERIOD_ENDING_DATE']>=predict_date].copy()
    #return masterDF3
...
#    
    ...
    if str(geo_id) =='142':
        ...
        output_DF = Simulation_function_Indonesia_v1.Simulation_Function_Indonesia(finalDF,price_flag,tdp_flag)
        #print(updated_df)
        #print(output_DF)
        ...
        output_DF = Simulation_function_BR.Simulation_Function_BR_Total(finalDF,price_flag,tdp_flag)
        #print(updated_df)
        #print(output_DF)
        output_DF['RECORD_TYPE'] = 'FORECASTED'
        ...
        output_DF = Simulation_function_neatherlands.Simulation_Function_neatherlands(finalDF,price_flag,tdp_flag)
        #print(updated_df)
        #print(output_DF)
        output_DF['RECORD_TYPE'] = 'FORECASTED'
        ...
        output_DF = Simulation_function_MX.Simulation_Function_MX_Total(finalDF,price_flag,tdp_flag)
        #print(updated_df)
        #print(output_DF)
        output_DF['RECORD_TYPE'] = 'FORECASTED'
        ...
        output_DF = Simulation_function_GERMANY.Simulation_Function_germany_Total(finalDF,price_flag,tdp_flag)
        #print(updated_df)
        #print(output_DF)
        output_DF['RECORD_TYPE'] = 'FORECASTED'
        ...
        if int(sub_catg_cd) in [1,2,6,9,10,11,13]:
           
            DF_online = finalDF.copy()
            DF_offline = finalDF.copy()
        
            online_remove_cols = ['PRICE_PER_VOL','SEASONALITY_INDEX_SALES','SALES_TREND_CAL','SEASONALITY_INDEX_PRICE',
                              'SEASONALITY_INDEX_TDP','PRICE_PER_VOL_offline', 'SEASONALITY_INDEX_SALES_offline',
                              'SEASONALITY_INDEX_TDP_offline', 'SEASONALITY_INDEX_PRICE_offline','SALES_TREND_CAL_offline']
            
            offline_remove_cols = ['PRICE_PER_VOL','SEASONALITY_INDEX_SALES','SALES_TREND_CAL','SEASONALITY_INDEX_PRICE',
                              'SEASONALITY_INDEX_TDP','PRICE_PER_VOL_online', 'SEASONALITY_INDEX_SALES_online',
                               'SEASONALITY_INDEX_PRICE_online','SALES_TREND_CAL_online']
         
            DF_online.drop(online_remove_cols,axis =1, inplace=True)
            DF_offline.drop(offline_remove_cols,axis =1,inplace=True)
            
            DF_online.columns = DF_online.columns.str.rstrip('_online')
            DF_offline.columns = DF_offline.columns.str.rstrip('_offline')
            
        
        elif int(sub_catg_cd) in [3,4]:
...
#            
        ...
        output_DF = Simulation_function_China.Simulation_Function_CHN_Total(DF_offline,DF_online,price_flag,tdp_flag)
        #print(updated_df)
        #print(output_DF)
        output_DF['RECORD_TYPE'] = 'FORECASTED'
        ...
        #Sanjay: Make a copy of finalDF (input) and keep only PERIOD_ENDING_DATE, PREDICTED_VOLUME & PRICE_PER_VOL, before any columns get renamed.
        tmp_out_df = finalDF[['PERIOD_ENDING_DATE', 'PREDICTED_VOLUME', 'PRICE_PER_VOL']]
        tmp_out_df.reset_index(inplace=True)
        #print("tmp_out_df:")
        #print(tmp_out_df.dtypes)
        #tmp_out_df.to_csv("san_input_DF.csv")

        finalDF.rename(columns = {'PREDICTED_VOLUME':'PREDICTED_VOLUME_OLD'},inplace =True)
        output_DF = Simulation_function_PH.Simulation_Function_PH_Total(finalDF,price_flag,tdp_flag)
        #print(updated_df)
        #print(output_DF)

        #Sanjay: Overwrite only those rows in tmp_out_df, where atleast one cell is changed by front-end user i.e., master_flag_df["ROW_SUM"] > 0 for that PERIOD_ENDING_DATE
        #print("output_DF:")
        #print(output_DF.dtypes)
        #output_DF.to_csv("san_output_DF1.csv")
        rows = tmp_out_df.shape[0]
        for i in range(0,rows):
            date_str = tmp_out_df.iloc[i]["PERIOD_ENDING_DATE"]

            #print("i=",i,"   date_str=",date_str,"   type=",type(date_str),"   RowSum=", master_flag_df.loc[date_str]['ROW_SUM'])

            if master_flag_df.loc[date_str]['ROW_SUM'] > 0:
                tmp_out_df.loc[i,'PREDICTED_VOLUME'] = output_DF.loc[output_DF['PERIOD_ENDING_DATE'] == date_str, 'PREDICTED_VOLUME'].iloc[0]
                tmp_out_df.loc[i,'PRICE_PER_VOL'] = output_DF.loc[output_DF['PERIOD_ENDING_DATE'] == date_str, 'PRICE_PER_VOL'].iloc[0]
                #print("PredictedVolume=", output_DF.loc[output_DF['PERIOD_ENDING_DATE'] == date_str, 'PREDICTED_VOLUME'].iloc[0])
                #print("PricePerVol=",output_DF.loc[output_DF['PERIOD_ENDING_DATE'] == date_str, 'PRICE_PER_VOL'].iloc[0])

        #Sanjay
        output_DF = tmp_out_df.copy(deep=True)
        output_DF['PERIOD_ENDING_DATE'] = output_DF['PERIOD_ENDING_DATE'].astype(str)
        ...
        #print("output_DF2:")
        #print(output_DF.dtypes)
        #output_DF.to_csv("san_output_DF2.csv")

    elif str(geo_id) =='169':
        
        output_DF = Simulation_function_FRANCE.Simulation_Function_FR_Total(finalDF,price_flag,tdp_flag)
        #print(updated_df)
        #print(output_DF)
        output_DF['RECORD_TYPE'] = 'FORECASTED'
        ...
        input_var_sim = None
        input_var_sim_rs = None
        input_var_ui_rs = None
        input_var_ui = None
...
#    
    ...
    output_DF['PREDICTED_VALUE'] = output_DF['PREDICTED_VALUE'].astype(float).round(2)
    ...
    channel_DF,format_DF, region_DF = ul_get_granular_data(geo_id,division,catg_cd,sub_catg_cd,forecast_scenario)
    calib_total_df = output_DF.copy()
    
    calib_total_df.rename(columns = {"PREDICTED_VOLUME":"VOLUME",
                                     "PREDICTED_VALUE":"VALUE",
                                     "PERIOD_ENDING_DATE":"MONTH_YEAR"},inplace=True)
    
    calib_total_df['VOLUME'] = calib_total_df['VOLUME'].astype(float)
    calib_total_df['VALUE'] = calib_total_df['VALUE'].astype(float)
    calib_total_df["MONTH_YEAR"] = calib_total_df["MONTH_YEAR"].apply(lambda x: datetime.datetime.strptime(x,'%Y-%m-%d'))
    #print(calib_total_df)
    
    if channel_DF.shape[0] >1:
        
        #to get channel calibration data
        channel_DF=simulation_calibration_v1.ul_generic_channel_calibration(calib_total_df,channel_DF,geo_id,catg_cd,sub_catg_cd)
        
        channel_DF["MONTH_YEAR"] = channel_DF["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
        #channel_DF = channel_DF[channel_DF['DATA_TYPE']=='FORECASTED']
        channel_DF["VOLUME"] = channel_DF["VOLUME"].astype(float).round(2)
        channel_DF["VALUE"] = channel_DF["VALUE"]*exch_rate
        channel_DF["VALUE"] = channel_DF["VALUE"].astype(float).round(2)
    ...
    if format_DF.shape[0] >1:
        ...
        #to get format calibration data
        format_DF=simulation_calibration_v1.ul_generic_format_calibration(calib_total_df,format_DF)
        
        format_DF["MONTH_YEAR"] = format_DF["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
        #format_DF = format_DF[format_DF['DATA_TYPE']=='FORECASTED']
        format_DF["VOLUME"] = format_DF["VOLUME"].astype(float).round(2)
        format_DF["VALUE"] = format_DF["VALUE"]*exch_rate
        format_DF["VALUE"] = format_DF["VALUE"].astype(float).round(2)
    ...
    if region_DF.shape[0] >1:
        ...
        #to get format calibration data
        region_DF=simulation_calibration_v1.ul_generic_region_calibration(calib_total_df,region_DF,geo_id,catg_cd,sub_catg_cd)
                
        region_DF["MONTH_YEAR"] = region_DF["MONTH_YEAR"].apply(lambda x: x.strftime('%m/%d/%Y'))
        #region_DF = region_DF[region_DF['DATA_TYPE']=='FORECASTED']
        region_DF["VOLUME"] = region_DF["VOLUME"].astype(float).round(2)
        region_DF["VALUE"] = region_DF["VALUE"]*exch_rate
        region_DF["VALUE"] = region_DF["VALUE"].astype(float).round(2)
...
def dummy_gen_sample_data():
    ...
    #input_vaiables= {"COUNTRY_NAME":"142","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"1","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
    #input_vaiables= {"COUNTRY_NAME":"73","DIVISION":"1","CATAGORY_NAME":"1","SUB_CATEGORY_NAME":"6","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
    #input_vaiables= {"COUNTRY_NAME":"181","DIVISION":"2","CATAGORY_NAME":"9","SUB_CATEGORY_NAME":"2","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
    #input_vaiables= {"COUNTRY_NAME":"179","DIVISION":"2","CATAGORY_NAME":"5","SUB_CATEGORY_NAME":"11","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
    #input_vaiables= {"COUNTRY_NAME":"170","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"1","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
    #input_vaiables= {"COUNTRY_NAME":"176","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"5","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
    #input_vaiables= {"COUNTRY_NAME":"175","DIVISION":"1","CATAGORY_NAME":"8","SUB_CATEGORY_NAME":"5","METRIC":"Volume", "FORECAST_TYPE":"TOTAL", "FORECAST_SCENARIO":"1"}
    ...
    request_header = {"request_header":input_vaiables}
    my_dict2.update(request_header)
        
    df1= pd.read_excel("simulation_demo_NL.xlsx",sheet_name="INPUT_VALUES")
    sales_vol_dict = {"INPUT_VALUES":(json.loads(df1.to_json(orient='records')))}
    my_dict2.update(sales_vol_dict)
    
    df2= pd.read_excel("simulation_demo_NL.xlsx",sheet_name="TABLE_VALUES")
...
################
#finalDF = ul_db_simulation_pred_sales_data(ret_json)
