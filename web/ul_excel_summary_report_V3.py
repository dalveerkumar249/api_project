...
def get_global_sales_data(input_values):
    ...
    select_query = """select ctry_name, div_desc,catg_desc,subcat_desc, MONTH_YEAR,sum(SALES_VALUE) from
                    (
                    select f.ctry_name,c.div_desc,e.catg_desc,b.subcat_desc, quarter(a.period_ending_date) as MONTH_YEAR,
                    ...
                    group by b.subcat_desc,e.catg_desc,f.ctry_name,c.div_desc, quarter(a.period_ending_date)
                    union
                    select f.ctry_name,c.div_desc,e.catg_desc,b.subcat_desc, quarter(a.period_ending_date) as MONTH_YEAR,
...
def ul_db_excel_summary_report(parameters):
    ...
    total_DF.to_csv("sample_summary2_final_server.csv")
    return total_DF
