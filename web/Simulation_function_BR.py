############ Brazil Total Level Prediction function "
...
def Simulation_Function_BR_Total(data,price_flag,tdp_flag):
    ...
    price_cfg= pd.read_excel("./br_models/BR_Model_Variables.xlsx",sheet_name='PRICE')
    tdp_cfg= pd.read_excel("./br_models/BR_Model_Variables.xlsx",sheet_name='TDP')
    sales_cfg= pd.read_excel("./br_models/BR_Model_Variables.xlsx",sheet_name='SALES')
    model_cfg= pd.read_excel("./br_models/BR_Model_Variables.xlsx",sheet_name='MODEL_NAME')
...
#    DATAF.rename(columns = {'AVG_TEMP_CELSIUS':'AVG(AVG_TEMP_CELSIUS)','HUMID_PCT':'AVG(HUMID_PCT)'},inplace =True)
    ...
    if (price_flag==0):
        ...
        filename = './br_models/Price_Total_Prediction_'+str(SUBCAT)+"_"+str(CHNL)+"_"+str(FMT)+"_"+str(REG)
    ...
    if tdp_flag==0:    
        ...
        filename = './br_models/TDP_Total_Prediction_'+str(SUBCAT)+"_"+str(CHNL)+"_"+str(FMT)+"_"+str(REG)
...
testing_data = pd.read_csv('./br_models/Brazil_TotalSales_MayonnaiseWithPriceAssumption.csv')

testing_data = pd.read_csv('./br_models/Brazil_TotalSales_FabricWithPriceAssumption.csv')

testing_data = pd.read_csv('./br_models/BrazilSkinCleasingTotalsales.csv')

testing_data = pd.read_csv('./br_models/Brazil_Total_Sales_Deo_Shampoo_Postwash6.csv')
...
output = Simulation_Function_BR_Total(testing_data,0,0)
